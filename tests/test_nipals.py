...
yarn_missing_scores = pd.read_csv(
    "tests/yarn_missing_scores.csv", index_col=0, usecols=[0, 2, 3, 4]
).values
yarn_missing_loadings = pd.read_csv(
    "tests/yarn_missing_loadings.csv", index_col=0
).values
...
def test_predict_from_pca():
    nip = nipals.Nipals(testdata)
    nip.fit(ncomp=2)
    assert nip.predict(pd.DataFrame([[63, 70, 98, 110, 124], [51, 82, 102, 110, 108]]))
    np.testing.assert_almost_equal(
...
def test_plot_classoptions():
    ...
    nip.fit(ncomp=2)
    plt = nip.plot(
        classlevels=["C1", "C2"], markers=["s", "o"], classcolors=["red", "black"]
    )
    assert isinstance(plt, matplotlib.figure.Figure)
    return plt
...
def test_pred_plot():
    nip = nipals.Nipals(testdata_class)
    nip.fit(ncomp=2)
    nip.predict(pd.DataFrame([[63, 70, 98, 110, 124], [51, 82, 102, 110, 108]]))
...
def test_plot_labels():
    ...
    nip.predict(
        pd.DataFrame(
            [[63, 70, 98, 110, 124], [51, 82, 102, 110, 108]], index=["a", "b"]
        )
    )
...
def test_pred_plot_labels():
    ...
    nip.predict(
        pd.DataFrame(
            [[63, 70, 98, 110, 124, 0, 1, 0], [51, 82, 102, 110, 108, 1, 0, 0]],
            index=["a", "b"],
...
def test_pls_missing_y():
    ...
    assert pls.fit(ncomp=2)
    np.testing.assert_almost_equal(
        pls.scores.values, oliveoil_missing_y_scores * [-1, 1], 3
    )
...
def test_dmodx_pca():
    ...
    np.testing.assert_almost_equal(
        nip.dModX(),
        [0.5364634, 0.3480396, 0.5702191, 1.5525099, 1.5752593, 0.3275259, 1.1255771],
    )
